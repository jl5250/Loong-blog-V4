...
export default function XuePage() {
  ...
  useGSAP(({ gsap, ScrollTrigger }) => {
    if (!snapRef.current || !wrapperRef.current) return;
    gsap.registerPlugin(ScrollTrigger);

    const panels = gsap.utils.toArray(".snap-panel") as HTMLElement[];
    ...
    if (portrait) {
      ...
      portraitRef.current?.addEventListener("mousemove", onMove);
      portraitRef.current?.addEventListener("mouseleave", onLeave);
      ...
      ScrollTrigger.create({
        ...
        onEnter: () => {
          gsap.to(el, {
            ...
            onUpdate: () => {
              const val = Math.floor(parseInt((el as HTMLElement).innerText.replace(/,/g, "")) || 0);
              (el as HTMLElement).innerText = val.toLocaleString();
            },
          });
        },
      });
    ...
    // Lyrics highlight
    const lLines = document.querySelectorAll<HTMLElement>(".lyric-line");
    if (lLines.length) {
      ScrollTrigger.create({
        trigger: ".lyrics-section",
        start: "top center", end: "bottom center", scrub: 1.5,
        onUpdate: (self: { progress: number }) => {
          const p = self.progress;
          const activeIdx = Math.min(Math.floor(p * lLines.length), lLines.length - 1);
          lLines.forEach((line, i) => {
            const dist = Math.abs(i - activeIdx);
            line.style.opacity = dist === 0 ? "1" : dist === 1 ? "0.5" : dist === 2 ? "0.2" : "0.06";
            line.style.transform = `scale(${dist === 0 ? 1.06 : 1}) translateY(${(i - activeIdx) * 6}px)`;
            line.style.color = dist === 0 ? "#b8860b" : "";
          });
        },
      });
    }
  ...
  return (
    <>
      <style>{`.snap-panel { min-height: 100vh; } @media (max-width: 768px) { .snap-panel { padding-left: 1rem; padding-right: 1rem; } }`}</style>
      <div ref={snapRef} style={{ height: `${PANELS * 100}vh`, position: "relative" }}>
        <div ref={wrapperRef} style={{ position: "relative", width: "100%", willChange: "transform", touchAction: "pan-y" }}>

          {/* ─── Panel 0: Hero ─── */}
          <section className="snap-panel relative flex items-center justify-center overflow-hidden"
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-4xl opacity-[0.08] tracking-[2rem] pointer-events-none select-none">👑 👑 👑</div>
            ...
            {/* Content */}
            <div className="relative z-[3] max-w-[640px] px-8 max-md:text-center max-md:mx-auto max-md:pt-16">
            ...
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-[3] flex flex-col items-center gap-2">
              <span className="text-[0.55rem] tracking-[0.3em] text-[#6b4c3b]">向下滚动 · 王者的征途</span>
              <div className="w-px h-9" style={{ background: "linear-gradient(to bottom, #b8860b, transparent)" }} />
            </div>
          </section>

          {/* ─── Panel 1: Photos ─── */}
          <section className="snap-panel flex flex-col items-center justify-center px-4 sm:px-6 py-16" style={{ background: "#faf6f0" }}>
          ...
          {/* ─── Panel 2: Three Acts ─── */}
          <section className="snap-panel flex flex-col items-center justify-center px-4 sm:px-6 py-16" style={{ background: "#f5f0e8" }}>
            <div className="max-w-[1000px] w-full">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {[{ icon: "⛓️", num: "第一幕 · 深渊", title: "困于兽笼，静待觉醒", desc: "暗黑锁链白发造型，巨型机械兽笼中挣扎。欲望与困兽的交织。" },
                  ...
                  <div key={i} className="p-6 rounded-[18px] text-center bg-white border border-[#e8dcc8] transition-all duration-300 hover:-translate-y-1.5 hover:shadow-[0_12px_40px_rgba(184,134,11,0.08)] hover:border-[rgba(184,134,11,0.2)]">
            ...
            <div className="max-w-[1000px] w-full">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-10">
                {[{ num: "5000", label: "㎡ 巨型四面台" }, { num: "5600", label: "㎡ 环形 LED 冰屏" }, { num: "1080", label: "组智能动态灯光" }, { num: "93", label: "米宽 × 36米高舞台" }].map((s, i) => (
                  <div key={i} className="p-5 text-center bg-white border border-[#e8dcc8] rounded-xl">
                    <div className="font-serif text-[1.8rem] font-black text-[#b8860b]"><span className="stat-num" data-target={s.num}>{s.num}</span></div>
          ...
          <section className="snap-panel flex flex-col items-center justify-center px-4 sm:px-6 py-16" style={{ background: "#f5f0e8" }}>
            <div className="max-w-[900px] w-full">
              ...
              <div className="lyrics-section text-center">
                <h3 className="font-serif text-[clamp(1rem,2vw,1.5rem)] font-bold mb-5">那些刻在心底的歌词</h3>
                {LYRIC_DATA.map((l, i) => (
          ...
          {/* ─── Panel 5: Footer ─── */}
          <footer className="snap-panel flex flex-col items-center justify-center px-6 bg-white border-t border-[#e8dcc8]">
            <p className="text-[0.85rem] text-[#6b4c3b] tracking-[0.1em] leading-8 text-center">
              Made with <span className="text-[#c0392b] inline-block" style={{ animation: "pulse 0.6s ease-in-out infinite alternate" }}>♥</span> by a 谦友
            </p>
            <p className="font-serif text-[0.65rem] text-[#b8860b] tracking-[0.3em] mt-2">承蒙关照 · 万兽之王</p>
          </footer>
        </div>
      </div>
